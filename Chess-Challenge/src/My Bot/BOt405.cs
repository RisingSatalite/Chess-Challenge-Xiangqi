...
public class MyBot405 : IChessBot
{
    //Attempt to combine 403 and 404 futureattack, so far even match if not worse against 404
    ...
    private int FutureAttackTotal(Board board, Move move)
    {
        ...
        try
        {
            return ((maxPower / 20) + (power / (possibleMoves.Length * 20)));
        }
        catch
        {
            return 0;
