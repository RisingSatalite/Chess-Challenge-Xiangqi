...
public class MyBot702 : IChessBot
{
    //using Bot 404
    ...
    private int SuperMoveJudger(Board board, Move move)
    {
        ...
        foreach (Move possibleMove in possibleMoves)
        {
            ...
            foreach (Move possibleMove2 in possibleMoves2)
            {
                ...
                powerAtt += potential;
                if (potential > powerAtt)
                {
                    maxAttPower = potential;
                }
                if (MoveIsCheckmate(board, possibleMove))
                {
                    mateAttack += 30;
                }
                board.MakeMove(possibleMove);
                ...
                foreach (Move ifMove2 in attackMoves2)
                {
                    ...
                    {
                        mateAttack -= 1;//When using machine add make better
                    }
                }
        ...
        try
        {
            return (mateAttack + capturedPieceValue - lostPieceValue + (maxAttPower / 50) + (powerAtt / (possibleMoves.Length * 10)));
        }
        catch
        {
