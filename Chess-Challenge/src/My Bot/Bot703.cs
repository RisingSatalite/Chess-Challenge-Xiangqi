...
//Add bot is performing horriblely for some reason, might remake it from what evilbot currently has
public class MyBot703 : IChessBot
{
    //using Bot 404
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        foreach (Move possibleMoves in allMoves)
        {
            ...
            int currentScore = (SuperMoveJudger(board, possibleMoves)//SOme sort of problem
                /*FutureAttackTotal(board, possibleMoves) +
                MateAble(board, possibleMoves) +
                MoveTakePower(board, possibleMoves) -
                MaxDangerDetection(board, possibleMoves)*/
                /*- FutureDefenceTotal(board, possibleMoves)*/
                /*FutureDefenceTotal2(board, possibleMoves)*/);
        ...
        {
            // If will get mated in one, or some other problem, choose a random move
    ...
    private int SuperMoveJudger(Board board, Move move)
    {
        //Future attack
        //Take power
        //Mateable
        ...
        foreach (Move possibleMove in possibleMoves)
        {
            ...
            //Piece capturedPiece = board.GetPiece(possibleMove.TargetSquare);
            ...
            if (capturedPiecePotential > lostPieceValue)
            {
                lostPieceValue = capturedPiecePotential;
            }
            //Bot turn
            foreach (Move possibleMove2 in possibleMoves2)
            {
                ...
                powerAtt += potential;
                if (potential > maxAttPower)
                {
                    maxAttPower = potential;
                }
                if (MoveIsCheckmate(board, possibleMove2))
                {
                    mateAttack += 30;
                }
                ...
                foreach (Move ifMove2 in attackMoves2)
                {
                    ...
                    {
                        mateTrap -= 1;//When using machine add make better
                    }
                }
        ...
        try
        {
            return (mateAttack + capturedPieceValue - mateTrap - lostPieceValue + (maxAttPower + 1 / 50) + (powerAtt + 1 / (possibleMoves.Length * 10)));
        }
        catch
        {
            return (maxAttPower / 10);
    ...
    private int FutureDefenceTotal2(Board board, Move move)
    {
        ...
        foreach (Move possibleMove in possibleMoves)
        {
            ...
            foreach (Move futureMove in futureBotMOves)
            {
                ...
                {
                    //Move[] beyondMoves = board.GetLegalMoves();
                    int potential = MoveTakePower(board, counterMove);
                    power += potential;
